...
void init_system(void) {
  ...
  init();
    ...
    scale.set_scale(-94.033927);
    scale.set_offset(411747);
}
...
int main(void) {
    ...
    while (1) {
        ...
        if (!(PIND & (1 << BTN_CONFIRM))) {
            ...
            if (!(PIND & (1 << BTN_CONFIRM))) {
                ...
                if (currentState == S_3) {
                    ...
                    measured_weight = 0;
                    s2_tare_at = millis() + 800;
                    currentState = S_2;
                    char status_buf[32];
                    if (isFifoStatus) sprintf(status_buf, "FIFO POS:%d", display_position);
                    else strcpy(status_buf, "CONFIRM");
        ...
        if (currentState == S_2) {
            if (s2_tare_at != 0 && millis() >= s2_tare_at) {
                s2_tare_at = 0;
                scale.tare();
                beep(100);
                ...
                else strcpy(tare_status, "CONFIRM");
                showOledLayout(tare_status, "WEIGHT:0.0G");
            }
            ...
            if (millis() - lastWeightUpdate > 500) { // 500ms마다 5회 평균 (노이즈 감소)
                ...
                // 값이 1g 이상 바뀔 때만 OLED 업데이트 (잦은 redraw 방지)
                static float last_oled_diff = -1.0f;
                char weight_buf[32];
                if (fabs(diff - last_oled_diff) >= 1.0f) {
                    last_oled_diff = diff;
                    char diff_str[8];
                    dtostrf(diff, 4, 1, diff_str);
                    sprintf(weight_buf, "WEIGHT:%sG", diff_str);
                    showOledLayout(status, weight_buf);
                }
                ...
                if (diff > 10.0f) { // 10g 이상의 변동이 생겼을 때 (노이즈 하한선)
                    if (fabs(diff - last_stable_weight) < 10.0f) { // 무게 차가 10g 이내면
                        if (stable_start_time == 0) {
                            stable_start_time = millis();
                        } else if (millis() - stable_start_time > 1000) { // 1초간 유지되면
                            ...
                            char mw_str[8];
                            dtostrf(measured_weight, 4, 1, mw_str);
                            sprintf(weight_buf, "WEIGHT:%sG", mw_str);
                            showOledLayout(status, weight_buf);
                            beep(150);
        ...
        // ── ⑦ S_4: SUCCESS 화면 고정 대기
        //          입고 버튼 → S_7 (버튼 핸들러 ③에서 처리)
        //          확인 버튼 → S_2 (버튼 핸들러 ⑤에서 처리)
        //          (폴링 없음 — 화면은 COUNT: 핸들러에서 이미 표시)
...
void handlePiCmd(const char* cmd) {
    if (strcmp(cmd, "DHT_REQ") == 0) {
        ...
        if (currentState == S_WAIT_COUNT) {
            ...
            dtostrf(measured_weight, 4, 1, mw_str);
            sprintf(weight_buf, "WEIGHT:%sG", mw_str);
            showOledLayout("SUCCESS", weight_buf);
            beep(150);
        }
    }
