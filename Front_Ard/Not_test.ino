...
typedef enum {
    ...
    S_2,                    // tare 완료 후 무게 감지 + 안정화 통합 상태
    S_3b,                   // 안정화 완료, 확인 버튼 대기
...
void  beep(int ms);
void  showOledLayout(const char* status_msg, const char* weight_msg);
...
void showOledLayout(const char* status_msg, const char* weight_msg) {
    ...
    if (status_msg && status_msg[0] != '\0') {
        uint8_t page = (weight_msg && weight_msg[0] != '\0') ? 2 : 3;
...
void init_system(void) {
  ...
  init();
    ...
    OLED_init();
    OLED_clear();
    showOledLayout("READY", "");

    // 무게센서 초기화
    scale.begin(5, 6); // DT=5, SCK=6
    scale.set_scale(20.0f);
    scale.tare();
}
    ...
    while (1) {
        ...
        if (!(PINB & (1 << (BTN_OUTBOUND - 8)))) {
            ...
            if (!(PINB & (1 << (BTN_OUTBOUND - 8)))) {
                ...
                if (currentState == S_IDLE) {
                    ...
                    dtostrf(w_init, 4, 1, w_str);
                    dtostrf(d_init, 4, 1, d_str);
                    char out_buf[48];
                    sprintf(out_buf, "OUTBOUND_REQ:W:%s,D:%s\n", w_str, d_str);
                    UART_print(out_buf);
                    ...
                    char w_str[8], d_str[8];
                    dtostrf(w_final, 4, 1, w_str);
                    dtostrf(d_final, 4, 1, d_str);
                    char out_buf[48];
                    sprintf(out_buf, "OUTBOUND_FINAL:W:%s,D:%s\n", w_str, d_str);
                    UART_print(out_buf);
                }
            }
        }
        ...
        if (!(PIND & (1 << BTN_CONFIRM))) {
            ...
            if (!(PIND & (1 << BTN_CONFIRM))) {
                ...
                if (currentState == S_3b) {
                    currentState = S_WAIT_COUNT;
                    char mw_str[8];
                    dtostrf(measured_weight, 4, 1, mw_str);
                    char out_buf[32];
                    sprintf(out_buf, "CONFIRM_WEIGHT:%s\n", mw_str);
                    UART_print(out_buf);
                }
            }
        }

        // ── ⑥ S_2: tare 완료 → 무게 감지 + 안정화 통합 (S_3b 진입까지 담당)
        if (currentState == S_2) {
            if (millis() - lastWeightUpdate > 500) {
                ...
                char status[32];
                if (isFifoStatus) sprintf(status, "FIFO POS:%d", display_position);
                else strcpy(status, "CONFIRM");

                static float last_oled_w  = -1.0f;
                static float last_stable  = -999.0f;
                static unsigned long stable_start = 0;

                if (w > 10.0f) {
                    // 실시간 표시
                    if (fabs(w - last_oled_w) >= 1.0f) {
                        last_oled_w = w;
                        char w_str[8], w_buf[32];
                        dtostrf(w, 4, 1, w_str);
                        sprintf(w_buf, "WEIGHT:%sG", w_str);
                        showOledLayout(status, w_buf);
                    }
                    // 안정화 판별
                    if (fabs(w - last_stable) < 10.0f) {
                        if (stable_start == 0) {
                            stable_start = millis();
                            ...
                            beep(150);
                            char w_str[8], w_buf[32];
                            dtostrf(measured_weight, 4, 1, w_str);
                            sprintf(w_buf, "WEIGHT:%sG", w_str);
                            showOledLayout(status, w_buf);
                            currentState = S_3b;
                        }
                    ...
                    // 10g 미만: 0g 표시, 안정화 리셋 (S_2 유지)
                    float display_w = (w < 8.0f) ? 0.0f : w;
                    if (fabs(display_w - last_oled_w) >= 1.0f) {
                        last_oled_w = display_w;
                        showOledLayout(status, "WEIGHT:0.0G");
                    }
                    last_stable  = -999.0f;
                    stable_start = 0;
                }
        ...
        // ── ⑦ S_4: DONE 표시 → 새 물건 감지 + 안정화 내부 처리 → S_3b
        //          입고 버튼은 안정화 여부 무관 S_7로 (버튼 핸들러 ③에서 처리)
        if (currentState == S_4) {
            if (millis() - lastWeightUpdate > 500) {
                lastWeightUpdate = millis();
                float w = scale.get_units(5);
                ...
                if (w > 50.0f) {  // 노이즈(±20g)와 실제 새 물건 구분
                    // 실시간 표시
                    if (fabs(w - last_oled_w) >= 1.0f) {
                        last_oled_w = w;
                        char w_str[8], w_buf[32];
                    ...
                    // 무게 없음: DONE 표시 유지, 안정화 리셋
                    if (last_oled_w != -1.0f || last_stable != -999.0f) {
                        last_oled_w  = -1.0f;
                        last_stable  = -999.0f;
                        stable_start = 0;
                        showOledLayout("DONE", "");
        ...
        // ── ⑦.6 S_3b: 안정화 완료, 물건 제거 감지 + 확인 버튼 대기
        if (currentState == S_3b) {
            if (millis() - lastWeightUpdate > 500) {
                lastWeightUpdate = millis();
                float w = scale.get_units(5);
                ...
                if (w < 10.0f) {
                    // 물건 제거 → S_2 복귀 (tare 불필요, PROGRESS:1,FIFO: 기준 유지)
                    currentState = S_2;
                    char status[32];
                    if (isFifoStatus) sprintf(status, "FIFO POS:%d", display_position);
                    else strcpy(status, "CONFIRM");
        ...
        if (currentState == S_IDLE && oledActive && millis() - oledClearAt > 3000) {
            OLED_clear();
            oledActive = false;
        }

        // ── ⑨ 온습도 30초 주기 전송 (S_IDLE 상태에서만 전송)
        if (currentState == S_IDLE && millis() - lastDhtSent > 30000) {
            float temp, hum;
            if (DHT22_read(&temp, &hum)) {
                char t_str[8], h_str[8];
                dtostrf(temp, 4, 1, t_str);
                dtostrf(hum, 4, 1, h_str);
                char out_buf[48];
                sprintf(out_buf, "ENV:T:%s,H:%s\n", t_str, h_str);
                UART_print(out_buf);
            }
            lastDhtSent = millis();
        }
    ...
    if (strcmp(cmd, "LED_Y:1") == 0) {
        ...
        if (currentState == S_1) {
            ...
            if (is_fifo) {
                ...
                showOledLayout(status_buf, "WEIGHT:0.0G");
                beep(100);
            } else {
                ...
                currentState = S_5;
                showOledLayout("CONNECT APP", "CONFIRM");
            }
        }
    }
        ...
        if (currentState == S_5) {
            currentState = S_6;
            char weight_buf[22];
            sprintf(weight_buf, "DEL:%d ITEMS", confirm_n);
            showOledLayout("CONFIRM", weight_buf);
            beep(100);
        }
    }
        ...
        if (currentState == S_OUTBOUND_WAIT_RESULT) {
            ...
            currentState = S_IDLE;
            char weight_buf[22];
            sprintf(weight_buf, "DEL:%d ITEMS", del_count);
            showOledLayout("CONFIRM", weight_buf);
            beep(150);
            _delay_ms(100);
            beep(150);
